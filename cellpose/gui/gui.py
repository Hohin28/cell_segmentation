...
class MainW(QMainWindow):
    ...
    def make_buttons(self):
        ...
        # ---- Cell Analysis Panel ---- #
        b += 1
        self.analysisBox = QGroupBox("Cell Analysis")
        self.analysisBox.setFont(self.boldfont)
        self.analysisBoxG = QGridLayout()
        self.analysisBox.setLayout(self.analysisBoxG)
        self.l0.addWidget(self.analysisBox, b, 0, 1, 9)

        widget_row = 0

        # Mode selector
        mode_label = QLabel("Mode:")
        mode_label.setFont(self.medfont)
        self.analysisBoxG.addWidget(mode_label, widget_row, 0, 1, 2)
        self.analysisModeDropDown = QComboBox()
        self.analysisModeDropDown.addItems(["auto", "blood", "papsmear", "histology", "generic"])
        self.analysisModeDropDown.setFont(self.medfont)
        self.analysisModeDropDown.setToolTip(
            "auto: auto-detect image type\n"
            "blood: WBC / RBC / Platelet (blood smear)\n"
            "papsmear: Pap smear cell types\n"
            "histology: Tumour / Lymphocyte / Stromal / Epithelial\n"
            "generic: label all cells as 'Cell'"
        )
        self.analysisBoxG.addWidget(self.analysisModeDropDown, widget_row, 2, 1, 5)

        # Run analysis button
        self.runAnalysisBtn = QPushButton("Analyse Cells")
        self.runAnalysisBtn.setFont(self.medfont)
        self.runAnalysisBtn.setToolTip("Classify cells, count, and extract boundaries")
        self.runAnalysisBtn.setEnabled(False)
        self.runAnalysisBtn.clicked.connect(self.run_cell_analysis)
        self.analysisBoxG.addWidget(self.runAnalysisBtn, widget_row, 7, 1, 2)

        widget_row += 1

        # Cell count display
        count_label = QLabel("Total cells:")
        count_label.setFont(self.medfont)
        self.analysisBoxG.addWidget(count_label, widget_row, 0, 1, 3)
        self.cellCountLabel = QLabel("—")
        self.cellCountLabel.setFont(self.boldfont)
        self.cellCountLabel.setStyleSheet("color: rgb(100, 220, 100);")
        self.analysisBoxG.addWidget(self.cellCountLabel, widget_row, 3, 1, 6)

        widget_row += 1

        # Classification summary (scrollable text area)
        from qtpy.QtWidgets import QTextEdit
        self.classificationText = QTextEdit()
        self.classificationText.setReadOnly(True)
        self.classificationText.setFont(self.smallfont)
        self.classificationText.setFixedHeight(110)
        self.classificationText.setStyleSheet(
            "background-color: rgb(30,30,30); color: rgb(200,200,200); border: 1px solid gray;"
        )
        self.classificationText.setPlaceholderText(
            "Run segmentation, then click 'Analyse Cells'."
        )
        self.analysisBoxG.addWidget(self.classificationText, widget_row, 0, 1, 9)

        widget_row += 1

        # Export annotated image button
        self.exportAnalysisBtn = QPushButton("Export Annotated Image")
        self.exportAnalysisBtn.setFont(self.smallfont)
        self.exportAnalysisBtn.setEnabled(False)
        self.exportAnalysisBtn.setToolTip("Save image with classification overlay to disk")
        self.exportAnalysisBtn.clicked.connect(self.export_analysis_image)
        self.analysisBoxG.addWidget(self.exportAnalysisBtn, widget_row, 0, 1, 9)

        widget_row += 1

        # ---- Selected Cell Info box ---- #
        sel_label = QLabel("Selected Cell  (click a cell on the image)")
        sel_label.setFont(self.smallfont)
        sel_label.setStyleSheet("color: rgb(150,150,150);")
        self.analysisBoxG.addWidget(sel_label, widget_row, 0, 1, 9)

        widget_row += 1
        self.selectedCellLabel = QLabel("\u2014  click any cell to identify it")
        self.selectedCellLabel.setFont(self.boldfont)
        self.selectedCellLabel.setStyleSheet(
        ...
        self.selectedCellLabel.setWordWrap(True)
        self.selectedCellLabel.setMinimumHeight(52)
        self.analysisBoxG.addWidget(self.selectedCellLabel, widget_row, 0, 1, 9)

        # internal state
        self._analysis_results = {}      # cell_id → classification dict
        self._analysis_boundaries = []   # list of boundary dicts
        self._analysis_mode_used = "auto"
    ...
    def select_cell(self, idx):
        ...
        Also updates the Cell Analysis 'Selected Cell' info box.
        """
        ...
        if self.selected > 0:
            ...
            self.update_layer()
            # ---- update Cell Analysis selected cell info ---- #
            self._update_selected_cell_info(idx)
    ...
    def compute_segmentation(self, custom=False, model_name=None, load_model=True):
        ...
        try:
            ...
            # ---- auto-run cell analysis after segmentation ---- #
            self._last_masks_for_analysis = masks.squeeze() if masks.ndim > 2 else masks
            if hasattr(self, 'runAnalysisBtn'):
                self.runAnalysisBtn.setEnabled(True)
            self.run_cell_analysis(auto=True)
    ...
    def _update_selected_cell_info(self, cell_id):
        """
        Update the 'Selected Cell' info box in the Cell Analysis panel
        whenever the user clicks a cell in the image.

        Args:
            cell_id (int): The mask label index of the clicked cell.
        """
        if not hasattr(self, 'selectedCellLabel'):
            return

        # If no analysis has been run yet, just show the ID
        if not self._analysis_results:
            self.selectedCellLabel.setText(
                f"Cell ID: {cell_id}\n"
                f"(Run 'Analyse Cells' to see classification)"
            )
            return

        info = self._analysis_results.get(cell_id)
        if info is None:
            self.selectedCellLabel.setText(
                f"Cell ID: {cell_id}  —  not in current analysis results"
            )
            return

        label      = info.get("label", "Unknown")
        area       = info.get("area", 0)
        compact    = info.get("compactness", 0.0)
        mean_int   = info.get("mean_intensity", 0.0)
        colour_rgb = info.get("colour", (200, 200, 200))

        # Build the label text
        text = (
            f"Cell {cell_id}  →  {label}\n"
            f"Area: {area} px   |   Compactness: {compact:.2f}   |   "
            f"Brightness: {mean_int:.2f}"
        )
        self.selectedCellLabel.setText(text)

        # Tint the border with the classification colour
        r, g, b = colour_rgb
        self.selectedCellLabel.setStyleSheet(
            f"background-color: rgb(20,20,20);"
            f"color: rgb({r},{g},{b});"
    ...
    def run_cell_analysis(self, auto=False):
        """
        Run classification + boundary extraction + count on the current masks.

        Called automatically after segmentation (auto=True) and manually
        via the 'Analyse Cells' button.
        """
        try:
            # get current masks
            if hasattr(self, '_last_masks_for_analysis'):
                masks = self._last_masks_for_analysis
            else:
                masks = self.cellpix[self.currentZ]

            if masks is None or masks.max() == 0:
                self.classificationText.setPlainText("No cells detected — run segmentation first.")
                return

            # get the raw image for intensity features
            image = self.stack[self.currentZ] if hasattr(self, 'stack') else None

            mode = self.analysisModeDropDown.currentText()

            # --- 1. Count ---
            n = cell_analysis.count_cells(masks)
            self.cellCountLabel.setText(f"<b>{n}</b>")

            # --- 2. Classification ---
            classification = cell_analysis.classify_cells(masks, image, mode=mode)

            # --- 3. Boundary extraction ---
            boundaries = cell_analysis.extract_boundaries(masks)

            # Store results for export
            self._analysis_results    = classification
            self._analysis_boundaries = boundaries
            self._analysis_mode_used  = mode

            # --- 4. Build summary text ---
            from collections import Counter
            label_counts = Counter(v["label"] for v in classification.values())
            lines = [f"Mode detected: {mode}", f"Total cells: {n}", ""]
            lines.append("Cell type breakdown:")
            for label, cnt in sorted(label_counts.items(), key=lambda x: -x[1]):
                pct = 100 * cnt / n if n > 0 else 0
                lines.append(f"  {label:<14} {cnt:>4}  ({pct:.1f}%)")

            if not auto:
                lines += ["", "Top 5 cells by area:"]
                top5 = sorted(classification.items(),
                               key=lambda x: x[1]["area"], reverse=True)[:5]
                for cid, info in top5:
                    lines.append(
                        f"  Cell {cid:>4}: {info['label']:<14}  "
                        f"area={info['area']} px  "
                        f"compact={info['compactness']:.2f}"
                    )

            self.classificationText.setPlainText("\n".join(lines))

            # Enable export
            if hasattr(self, 'exportAnalysisBtn'):
                self.exportAnalysisBtn.setEnabled(True)

            self.logger.info(f"Cell analysis complete: {n} cells, mode='{mode}'")

        except Exception as e:
            self.logger.error(f"Cell analysis error: {e}")
            if hasattr(self, 'classificationText'):
                self.classificationText.setPlainText(f"Analysis error: {e}")


    def export_analysis_image(self):
        """Save a copy of the current image with classification overlay to disk."""
        try:
            from qtpy.QtWidgets import QFileDialog
            if not self._analysis_results:
                print("GUI_INFO: run cell analysis first before exporting.")
                return
            ...
            annotated = cell_analysis.draw_analysis_overlay(
                image.astype(np.uint8),
                masks,
                self._analysis_results,
                self._analysis_boundaries,
                show_labels=True,
            )
            ...
            save_path, _ = QFileDialog.getSaveFileName(
                ...
                default_name + "_analysis.png",
                "PNG Images (*.png);;TIFF Images (*.tif)"
            )
            if save_path:
                imsave(save_path, annotated)
                print(f"GUI_INFO: annotated image saved to {save_path}")

        except Exception as e:
            print(f"GUI_ERROR: export failed: {e}")
